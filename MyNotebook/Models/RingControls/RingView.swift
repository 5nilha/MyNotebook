...
class RingView: UIView {
    ...
    var stateClosures = [RingControlState: () -> Void]()
    var selected = false
    var fannedOut = false
    ...
    var locationState: (() -> Void)? {
        if selected {
            ...
            let transform = fannedOut ? CGAffineTransform.identity : CGAffineTransform(scaleX: 0.01, y: 0.01)
